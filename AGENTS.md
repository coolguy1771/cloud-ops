...
- Hubble UI (`hubble.cloud.witl.xyz`) is Cilium Hubble UI + Relay (ClusterIP); browser OIDC via `istio-ecosystem/authservice` in `kubernetes/apps/authservice/` (istiod `extensionProviders.authservice-grpc`); Authentik app `hubble` / group `cloud-ops-admin`; vault item `hubble-oauth`; setup script `kubernetes/apps/authservice/scripts/setup-authentik-hubble.sh`.
